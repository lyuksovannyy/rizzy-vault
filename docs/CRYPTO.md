...
**Decided 2026-09-25: yes, mandatory for every account, with the derivation and the Emergency Kit shipping in M1** ([ADR 0004](adr/0004-key-derivation-argon2id-secret-key.md), [§16](#16-open-questions-for-the-owner) question 1). ROADMAP §4.3 said "M1 decision, M3 ship"; its row now reads "M1 (derivation, Emergency Kit) / M3 (QR transfer, polish)". Only UX polish, such as QR transfer between devices, is left for M3. The reasons, as recommended:
...
An optional SK would double the code paths and the analysis. **Rejected.** The owner decided on 2026-09-25 that the SK is mandatory; see [§16](#16-open-questions-for-the-owner) question 1.
...
The owner decided every question below on 2026-09-25, in each case as recommended. The reasoning is kept.

1. **Decided 2026-09-25:** mandatory for every account from M1. The derivation and the Emergency Kit ship in M1; only QR transfer and polish are left for M3 ([ADR 0004](adr/0004-key-derivation-argon2id-secret-key.md)). **Secret Key in M1, mandatory.** ROADMAP said "M1 decision, M3 ship". *Recommendation:* ship the derivation and the Emergency Kit in M1, mandatory for all accounts, and leave only QR transfer and polish for M3 ([§7](#7-secret-key)). Without it, goal 2 is false for any account with a guessable password.
2. **Decided 2026-09-25:** on by default, with the opt-out; a server-enforced 72 h wait, admin-configurable from 0 to 30 days, that any enrolled device can cancel; every device and the email are notified ([ADR 0008](adr/0008-account-recovery.md)). **Recovery code on by default, behind a waiting period.** *Recommendation:* on by default, with an opt-out that shows "forgetting your password = data loss". Use it with a server-enforced **72 h waiting period** that any enrolled device can cancel, and notify every device and the email. Admins can lower the wait, down to 0 for single-user instances. Without the wait, kit + server access = immediate account takeover (threat model Q-15).
3. **Decided 2026-09-25:** keep `kdf_id` 1 = 64 MiB / t3 / p4 even at 1–2 s, subject to the M1 full-login spike below ([ADR 0004](adr/0004-key-derivation-argon2id-secret-key.md)). **Freeze `kdf_id` 1 = 64 MiB / t3 / p4** only after the M1 low-end phone measurement. *Recommendation:* keep it even at 1–2 s on old phones. The keystore path (`E_ks`) helps only an enrolled device and AutoFill. The first login on a phone runs the OPAQUE KSF at the account's `kdf_id` in the main app. The M1 spike therefore measures a full OPAQUE login (KSF plus local wrap) in the main-app process on the lowest-end target phones. If that runs out of memory, `kdf_id` 1 is changed before any M1 account exists, or a Server-mode enrolment path that approves a new phone from an existing device is specified first ([§6.4](#64-feasibility)).
4. **Decided 2026-09-25:** NFC, no trimming; unassigned code points are rejected in new master passwords; every `unicode-normalization` bump is reviewed like a crypto bump. **Password normalisation: NFC, no trimming.** *Recommendation:* NFC. Other password managers differ (U), which matters only for importing *master* passwords. We never do that. A related choice: code points that are unassigned in the pinned Unicode tables could change their NFC form once assigned. *Recommendation:* reject unassigned code points in new master passwords (this needs a Unicode general-category table, and which crate provides it is U, confirm in M1), and review every `unicode-normalization` bump like a crypto bump ([ADR 0009](adr/0009-crypto-dependency-policy.md)).
5. **Decided 2026-09-25:** opt-in checkbox, default off. **Remember the SK in the web vault's browser storage.** *Recommendation:* opt-in checkbox, default off.
6. **Decided 2026-09-25:** yes, every op is signed from M1 ([ADR 0006](adr/0006-key-hierarchy.md)). **Sign every op from M1.** It costs one Ed25519 signature per op and adds complexity, but M9 needs it and the op format would otherwise change later. *Recommendation:* yes.
7. **Decided 2026-09-25:** yes, nightly fuzzing in a separate scheduled CI job, weekly and non-blocking for PRs ([ADR 0009](adr/0009-crypto-dependency-policy.md)). **Fuzzing on nightly** in a separate scheduled CI job ([§15](#15-testing)). *Recommendation:* yes, weekly, and non-blocking for PRs.
8. **Decided 2026-09-25:** yes, AES-256-GCM-SIV stays a reserved id (0x02) with no code. **AES-256-GCM-SIV** stays a reserved id with no code. *Recommendation:* yes. Revisit only if a platform shows XChaCha20 is too slow, which we do not expect.
9. **Decided 2026-09-25:** yes, Padmé padding from M1 ([ADR 0005](adr/0005-symmetric-encryption-aead.md)). **Padmé padding for items from M1** (ROADMAP listed size padding as a Should in M4). *Recommendation:* yes in M1. It is free now and a format change later.
10. **Decided 2026-09-25:** full rotation for a lost or stolen device, standard rotation for one wiped and handed over; the UI asks which ([ADR 0006](adr/0006-key-hierarchy.md)). **Full rotation (including identity keys) as the default on device revocation.** *Recommendation:* yes for "lost/stolen", standard for "wiped and handed over". The UI asks which.
11. **Decided 2026-09-25:** yes, rotate by default on recovery and on an SK change, with an explicit "skip rotation" opt-out; a plain password change only offers rotation ([ADR 0008](adr/0008-account-recovery.md)). **Rotate the account key by default on recovery and on an SK change** ([§11.5](#115-master-password-or-secret-key-change), [§11.9](#119-recovery-with-the-emergency-kit)). Both flows assume the old kit is exposed, and old `E_rec` / `E_srv` copies in DB backups would otherwise keep yielding the current account key. *Recommendation:* yes, with an explicit "skip rotation" opt-out. A plain password change keeps "also rotate keys" as an offer, not a default.
12. **Decided 2026-09-25:** split the link token from the access token; only failures with a valid link token count toward the 10-attempt burn, and everything else is rate-limited per IP. **Share burn rule** (threat model Q-20). *Recommendation:* split the link token from the access token ([§11.11](#1111-public-share-link-opening-m5)): only failures with a valid link token count toward the 10-attempt burn, and everything else is rate-limited per IP.
13. **Decided 2026-09-25:** ASCII-only login names for v1.0. **ASCII-only login names** (`[a-z0-9._+@-]`, [§2](#2-conventions)). Internationalised names would need Unicode case folding, whose tables change between releases, on the enumeration-sensitive lookup path. *Recommendation:* ASCII for v1.0.
14. **Decided 2026-09-25:** yes, the `smtp` role keeps a pin store. **A pin store for the `smtp` role** (M6, [§11.13](#1113-mail-ingress-m6)). It is a small file on `smtp`'s own volume, which [ADR 0010](adr/0010-server-shape.md) §4 provides; it holds pins, no secrets and no DB data. *Recommendation:* yes. Without it, whoever controls `api` or the DB can substitute every account's mail key.
