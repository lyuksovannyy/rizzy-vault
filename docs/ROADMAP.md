...
| Pri | Item | When |
|---|---|---|
| M | Modular monolith: one Rust binary (`axum` + `tokio` + `sqlx`) with selectable roles (`api`, `web`, `notify`, `worker`, `smtp`, `icons`). See [ADR 0010](adr/0010-server-shape.md) | M1 |
...
| W | Microservices with a DB per service / separate images per service | Won't. Split further only on the criteria in [ADR 0010](adr/0010-server-shape.md#alternatives-considered) |
